...
class ServiceUser {
  ...
  async update(user, field, value, transaction) {
      ...
      if(!(allowedFields.includes(field))) {
        throw this.error(`It is not allowed to change the ${field} field`)
      } else if(!user.id) {
        throw this.error('invalid user')
      } if (field === 'password') {
        const hashedPass = await this.security.hash(value, salt)
        return this.repository.update(user, field, hashedPass, transaction)
      }
...
async updateRole(user, newRole, transaction) {
  if(!user.id) {
    throw this.error('invalid user')
  } else if(!(newRole === "employee" || newRole === "admin")) {
    throw this.error('invalid new role')
  }
  ...
  async login(email, password, transaction) {
    ...
    const session = await this.serviceSession.create(token, user.id, transaction);
    return {
      token,
