...
describe('delete organization', () => {
 ...
 it('success', async () => {
  const findOrganization = await serviceOrganization.findOne(organization.id, transaction) 
  const deletedOrg =  await serviceOrganization.delete(findOrganization, transaction)
  const findDeleteOrganization =  serviceOrganization.findOne(organization.id, transaction)
  
  await expect(deletedOrg.id).toBe(organization.id)
  await expect(findDeleteOrganization).rejects.toThrow('no organization in this id')
 })

 
 it('fail for invalid organization', async () => {
  const deletedOrg =  serviceOrganization.delete('organization', transaction)
  
  await expect(deletedOrg).rejects.toThrow('organization not found')
 })
})
